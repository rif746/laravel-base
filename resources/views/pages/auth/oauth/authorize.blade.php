    ...
    <div class="card" style="max-width:460px; width:100%;">
        <div class="card-body p-5">
            ...
            <div class="mb-4 text-center">
                ...
                <h1 class="card-title h5 mb-1">{{ __('domains/auth.pages.oauth.authorize.header') }}</h1>
                <p class="small text-muted mb-0">{{ __('domains/auth.pages.oauth.authorize.subheader') }}</p>
            </div>
            ...
            <div class="d-flex align-items-center gap-3 mb-3">
                <div class="flex-shrink-0 bg-primary-subtle rounded-circle d-flex align-items-center justify-content-center"
                    ...
                    <i class="ti ti-plug-connected fs-5 text-primary"></i>
                </div>
                <div>
            ...
            {{-- Approve --}}
            <form method="POST" action="{{ route('passport.authorizations.approve') }}" class="mb-2">
                @csrf
                <input type="hidden" name="state" value="{{ $request->state }}">
                <input type="hidden" name="client_id" value="{{ $client->getKey() }}">
                <input type="hidden" name="auth_token" value="{{ $authToken }}">
                <button type="submit" class="btn btn-primary w-100">
                    <i class="ti ti-check me-1"></i> {{ __('domains/auth.fields.oauth.authorize.submit') }}
                </button>
            </form>
